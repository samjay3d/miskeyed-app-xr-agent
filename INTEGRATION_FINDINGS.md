...
## F-018 — Production runtime contract is NGC PB 26h1, not the template feature branch

**Status:** Resolved in CI.

`kit-app-template` remains useful project/build scaffolding, but its default
download is a feature-branch development runtime. The supported application
target is Kit SDK Production Branch 26h1 from NVIDIA NGC. CI authenticates the
official NGC CLI with the single `NGC_API_KEY` secret and downloads the pinned
Linux and Windows PB 26h1 resources at version 110.1.2 outside the source tree.
There are no copied browser URLs or mutable checksum variables.
`kit-sdk.toml` records the product branch, NGC collection, resource/version,
and independently pinned template revision.

The PyPI matrix does not install Kit or `miskeyed-xr-agent`; it verifies only
that the launcher is a dependency-free `py3-none-any` wrapper. The separate Kit
job uses Kit's embedded Python to install the released native core wheel.

## F-019 — NGC SDK layout is not the Kit App Template output layout

**Status:** Fixed after run `32629700152`.

**Observed:** The NGC CLI successfully downloaded the 2.83 GB Linux 110.1.2
resource, but CI assumed the template build layout `release/kit/kit`. The NGC
SDK archive has its own root layout, so discovery returned an empty path. The
Windows leg also used the obsolete `ngccli_win.zip` convenience URL, which
returned an HTML response rather than a ZIP.

**Boundary decision:** Locate the kernel executable by its platform filename,
derive the embedded Python directory from the executable's actual directory,
and only then derive the app/extension root. Windows downloads the pinned NGC
CLI executable from the NGC `ngc_cli` resource API. CI prints a bounded SDK
layout inventory so future NGC packaging changes are diagnosable in one run.
